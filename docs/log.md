...
| `[INFO] [server_manager] [update] SteamCMD: ...` | Manager command output (piped) | `run_logged` / `run_hook_logged` -> `log_pipe` | Printed raw without manager timestamp. Not filtered by `LOG_LEVEL`. Context label is set by the caller (e.g., `update`, `backup`). |
| `[INFO] [server_manager] [server-log] [Session] 'HostOnline' (up)!` | Enshrouded server log file (streamed) | `server-manager-logstream` or `manager.sh logs` | Printed raw without manager timestamp. Controlled by `LOG_TO_STDOUT`, `LOG_TAIL_LINES`, `LOG_POLL_INTERVAL`, `LOG_FILE_PATTERN`. |
| `[WARN] [server_manager] [supervisor] supervisord: 2026-02-06 17:45:50,123 WARN ...` | Supervisor log file (streamed) | `server-manager-supervisor-logstream` | Printed raw without manager timestamp. The supervisord line itself may include its own timestamp. Active only when `LOG_TO_STDOUT=true`. |
| `[ERROR] [server_manager] [syslog] 2026-02-06T17:45:40+00:00 TAG: Message` | Syslog (streamed) | `server-manager-syslog` + `server-manager-syslog-logstream` | Printed raw without manager timestamp. Syslog timestamp is embedded in the message. Active only when `LOG_TO_STDOUT=true` and `rsyslogd` exists. Severity is mapped to debug/info/warn/error. |
