...
- The container entrypoint now **bootstraps and then execs `supervisord`**, making supervisor PID1 and keeping the manager supervised. 【F:Dockerfile†L128-L131】
...
**Detail:** `run` starts the manager loop (supervised), `setup` prepares config/dirs/A2S, `status` shows manager/server/version/player state, and `logs` tails the newest log file. 【F:server_manager/manager.sh†L21-L208】
...
- A **log streamer** tails the newest log file and switches on rotation via polling. 【F:server_manager/lib/server.sh†L569-L618】
- Streaming is always enabled and routed to stdout. 【F:server_manager/lib/common.sh†L282-L285】【F:docs/log.md†L1-L34】

**Detail:** Streaming is always enabled; manager core logs still print to stdout. 【F:server_manager/lib/server.sh†L621-L633】【F:docs/log.md†L1-L34】
...
- **Syslog streaming** is supported via `rsyslogd` and dedicated supervisor programs, and streamed output is forwarded to stdout. 【F:server_manager/manager.sh†L152-L287】【F:server_manager/supervisord.conf†L69-L145】【F:docs/log.md†L1-L34】
...
- Log streaming is always enabled and routed to stdout. 【F:server_manager/lib/common.sh†L282-L285】【F:docs/log.md†L1-L34】
...
- **Entrypoint migration**: The old entrypoint script is replaced by a bootstrap that execs supervisor PID1. 【F:Dockerfile†L128-L131】【F:server_manager/manager.sh†L647-L659】
